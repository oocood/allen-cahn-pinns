# warm up has make great progress in convergence in pinns, this program is using to load pinns neural network that has pre-trained
# to initialize weight in nerual network to train more instabe para group, such as big diffusion coef or big reaction coef
# written by fuyi li on wednesday 12 18 2024, all right reserved 
...
class PINNs(nn.Module): # The features mat shape is N*3 
    ...
    def loss_pde(self, x_train):
        ...
        u_xx = u_xx_yy_tt[:, [0]]
        u_yy = u_xx_yy_tt[:, [1]]
        f = u_t - kappa**2*(u_xx+u_yy)+5*(u**3-u) # pde form 5 using aiming to accerate phase separation
        pde_residual = f
...
x_data_path = './ac_data_para_5/x_axis.txt'
y_data_path = './ac_data_para_5/y_axis.txt'
u_data_path =  './ac_data_para_5/imex/'
...
# initial_xy = initial_xy[::5]
# initial_u = initial_u[::5]
...
start_epoch = pinnsnet.load_checkpoint('./ac_2d_model_para_1/checkpoint_700.pth') # direct train may drop into trival solution
optimizer_adam = optim.Adam(pinnsnet.parameters(), lr=0.0001)
switch_epoch = 10000
criterion = nn.MSELoss()

# start iteration loop
train_loss_save_path = './train_loss_without_warmup.txt'
test_loss_save_path = './test_loss_without_warmup.txt'
num_epoches = 20000
test_interval = 20 # test mode accuracy every 5 epoches
save_interval = 50
save_dir = './ac_2d_model_para_5'
print('Iteration begins!')
with open(train_loss_save_path, 'w') as f1, open(test_loss_save_path, 'w') as f2:
    for epoch in range(num_epoches):
        ...
        for _, x_train in enumerate(PDE_Dataloader):
            ...
            if epoch<3000:
                ...
                continue
        np.savetxt(f1, np.array([[loss.detach().cpu().numpy(), loss_ic.detach().cpu().numpy(), loss_bc.detach().cpu().numpy()]]), fmt='%f')
        if (epoch+1) % 10 == 0:
        ...
        if epoch % test_interval == 0:
            ...
            with torch.no_grad():
                ...
                np.savetxt(f2, np.array([average_error.cpu().numpy()]), fmt='%f')
